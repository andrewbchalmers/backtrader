...
def CalculateLookback(strategy_class, strategy_params=None):
    ...
    # Add 20 for the ATR SMA calculation (20-period moving average)
...
# Define strategy parameters - Based on research findings
strategy_params = {
    'aroon_len': 20,
    'atr_entry_len': 10,
    'atr_entry_mult': Decimal("2.0"),  # From ATR(10,2) - not used in filter
    'atr_entry_sma_period': 20,  # ATR is "low" when < its 20-period SMA
    'atr_filter_mult': Decimal("1.2"),  # Threshold: 1.0 = exact, 1.2 = 20% looser (try 1.2-1.5)
    ...
    'verbose': True  # Enable to see entry/exit signals
}
...
cerebro.addstrategy(Strategy,
                    aroon_len=strategy_params['aroon_len'],
                    atr_entry_len=strategy_params['atr_entry_len'],
                    atr_entry_mult=strategy_params['atr_entry_mult'],
                    atr_exit_len=strategy_params['atr_exit_len'],
                    atr_exit_mult=strategy_params['atr_exit_mult'],
                    stop_loss_pct=strategy_params['stop_loss_pct'],
                    take_profit_pct=strategy_params['take_profit_pct'],
                    use_take_profit=strategy_params['use_take_profit']
                    )
...
if strategy_params['use_atr_filter']:
    print(f"  Entry: AROON({strategy_params['aroon_len']}) crossover when ATR({strategy_params['atr_entry_len']}) < SMA({strategy_params['atr_entry_sma_period']}) (calm market)")
else:
    print(f"  Entry: AROON({strategy_params['aroon_len']}) crossover (ATR filter DISABLED)")
print(f"  Exit: ATR({strategy_params['atr_exit_len']}, {strategy_params['atr_exit_mult']}) trailing stop + {float(strategy_params['stop_loss_pct'])*100}% SL")
if strategy_params['use_take_profit']:
    print(f"        + {float(strategy_params['take_profit_pct'])*100}% TP")
print()
...
# Calculate and plot AROON for test period
...
aroon_up, aroon_down = calculate_aroon(df['high'].values, df['low'].values, strategy_params['aroon_len'])
...
# Create a second y-axis for AROON
ax1_aroon = ax1.twinx()
...
ax1.set_title(f'{symbol} - Price Chart with AROON Indicator (Test Period Only - Indicators Fully Loaded)', fontsize=14, fontweight='bold', color='white')

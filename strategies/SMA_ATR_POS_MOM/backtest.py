...
strategy_params = {
    ...
    'slow_len': 50,
    'atr_len': 10,
    'trend_len': 200,  # Added to ensure lookback calculation includes this
...
cerebro.addstrategy(Strategy,
                    ...
                    trend_len=strategy_params['trend_len'],
...
from_start_trend = df['close'].rolling(window=strategy_params['trend_len']).mean()

fast_sma_test = from_start_fast.iloc[test_start_idx:]
slow_sma_test = from_start_slow.iloc[test_start_idx:]
trend_sma_test = from_start_trend.iloc[test_start_idx:]
...
ax1.plot(test_dates, trend_sma_test, color='#ffa500', linewidth=2, label=f'SMA {strategy_params["trend_len"]} (Trend)', alpha=0.7)

...
CSV_FILE = '../bullish_screen_stocks.csv'
PERIOD = "2y"
INITIAL_CASH = 10_000
PLOT_BEST = True
# ============================================================================
...
def run_multi_backtest(csv_file='stocks.csv', period="2y", initial_cash=10_000,
    ...
    print(f"\n{'='*70}")
    print(f"MULTI-STOCK BACKTEST - LORENTZIAN CLASSIFICATION (TREND FEATURES)")
    print(f"{'='*70}\n")
    ...
    if strategy_params:
        ...
        print(f"  Features:")
        print(f"    - F1: {strategy_params.get('f1_type', 'RSI')}({strategy_params.get('f1_param_a', 14)})")
        print(f"    - F2: {strategy_params.get('f2_type', 'ADX')}({strategy_params.get('f2_param_a', 14)})")
        print(f"    - F3: {strategy_params.get('f3_type', 'ATRR')}({strategy_params.get('f3_param_a', 14)})")
        print(f"    - F4: {strategy_params.get('f4_type', 'PP')}({strategy_params.get('f4_param_a', 20)})")
        print(f"    - F5: {strategy_params.get('f5_type', 'ER')}({strategy_params.get('f5_param_a', 10)})")
        print(f"  Filters:")

...
CONFIG = {
    ...
    'param_grid': {
        ...
        'min_raw_prediction': [0.0],  # Disabled — normalized prediction strength is the primary gate
                                     # With K=10 & label_dead_zone=0.225: min non-zero raw≈0.225.
                                     # Meaningful values: 2.0≈avg 0.2ATR, 4.0≈avg 0.4ATR, 6.0≈avg 0.6ATR.
        ...
        # Momentum acceleration: short momentum > medium momentum = bullish acceleration
        # Replaced MTD(8,252) which used year-over-year comparison causing persistent
        # bearish signals during recovery rallies (e.g., AMD May-July 2025)
        'f3_type': ['MACC'],
        ...
        # ==================== PYRAMIDING ====================
        # When the ML fires another valid buy signal while already long, add to the position.
        # Uses identical entry conditions (signal_ok, kernel/EMA/SMA filters) — the ML decides.
...
class TradeRecorder(bt.Analyzer):
    ...
    def start(self):
        ...
        self._open_trade_norm_prediction = 0.0
        self.daily_dates = []
        self.daily_values = []
    ...
    def notify_trade(self, trade):
        if trade.justopened:
            ...
            # Capture ML prediction strength at entry (already set in _execute_buy)
            self._open_trade_norm_prediction = getattr(self.strategy, 'entry_norm_prediction', 0.0)
        ...
        if trade.isclosed:
            ...
            self.trade_log.append({
                ...
                'entry_norm_prediction': self._open_trade_norm_prediction,
            })
            self._open_trade_entry = None
            self._open_trade_price = None
            self._open_trade_size = None
...
PARAM_NAMES = [
    ...
    # Pyramiding
    'use_pyramiding', 'max_pyramid_entries', 'pyramid_size_pct', 'pyramid_min_profit_atr',
    # Other
...
def run_walkforward_optimization(config):
    ...
    # ── PASS 1: Run IS optimization for ALL periods before selecting anything ─
    # Collects IS results across all periods so we can pick the param set that is
    # MOST CONSISTENTLY GOOD on average, rather than the luckiest single-period winner.

    print(f"\n{'='*70}")
    print(f"PASS 1/2: IN-SAMPLE OPTIMIZATION (ALL PERIODS)")
    print(f"{'='*70}")

    all_train_results = {}   # period_idx -> {params_tuple: aggregated_result}
    all_train_grouped = {}   # period_idx -> {params_tuple: [per-stock results]}

    for period_idx, period_info in enumerate(periods):
        train_start, train_end, test_start, test_end = period_info

        print(f"\n{'='*70}")
        print(f"PERIOD {period_idx + 1}/{len(periods)} — IN-SAMPLE PHASE")
        print(f"{'='*70}")
        print(f"Train: {train_start} to {train_end}")
        print(f"Test:  {test_start} to {test_end} (held out)")
        ...
        all_train_results[period_idx] = train_results
        all_train_grouped[period_idx] = train_grouped

        # Show per-period IS ranking (informational only — final selection happens after all periods)
        sorted_is = sorted(train_results.items(), key=lambda x: x[1][ranking_metric], reverse=True)
        print(f"\n   IS results ranked by {ranking_metric} (top 5 of {len(sorted_is)}):")
        print(f"   {'Rank':<5} {'Score':>10}  {'AvgRet%':>8}  {'Sharpe':>7}  {'Trades':>7}  (fc=feature_count  ll=label_lookahead  k=neighbors)")
        for rank_i, (pt, rv) in enumerate(sorted_is[:5], 1):
            pd_ = rv['params']
            print(f"   {rank_i:<5} {rv[ranking_metric]:>10.4f}  {rv['avg_return_pct']:>+7.2f}%  "
                  f"{rv['avg_sharpe']:>7.3f}  {int(rv['total_trades']):>7}  "
                  f"fc={pd_.get('feature_count', '?')} "
                  f"ll={pd_.get('label_lookahead', '?')} "
                  f"k={pd_.get('neighbors_count', '?')}")

    n_valid_periods = len(all_train_results)
    if n_valid_periods == 0:
        print(f"\n❌ No valid IS results across any period. Exiting.")
        sys.exit(1)

    # ── GLOBAL PARAMETER SELECTION ────────────────────────────────────────────
    # For each param set, compute average IS score across all periods in which it
    # passed quality filters. Prefer sets that qualified in the most periods.
    # Param sets that were only lucky in one period are penalised: missing periods
    # are filled with that set's own worst-period score before averaging.

    print(f"\n{'='*70}")
    print(f"GLOBAL PARAMETER SELECTION  (avg IS {ranking_metric} across all {n_valid_periods} periods)")
    print(f"{'='*70}")
    ...
    def _passes_qf(v):
        return (v['total_trades'] >= qf['min_total_trades']
    ...
    # Collect per-period scores (quality-filtered)
    param_period_scores = {}  # params_tuple -> {period_idx: score}
    for period_idx, train_results in all_train_results.items():
        for k, v in train_results.items():
            if _passes_qf(v):
                param_period_scores.setdefault(k, {})[period_idx] = v[ranking_metric]

    # Prefer params that qualified in ALL periods
    full_coverage = {k: s for k, s in param_period_scores.items() if len(s) == n_valid_periods}
    if full_coverage:
        candidate_pool = full_coverage
        print(f"   {len(full_coverage)} param sets passed quality filters in all {n_valid_periods} IS periods")
    else:
        majority = max(1, (n_valid_periods + 1) // 2)
        partial = {k: s for k, s in param_period_scores.items() if len(s) >= majority}
        if partial:
            candidate_pool = partial
            print(f"   ⚠ No param set qualified in all periods. "
                  f"Using {len(partial)} sets that qualified in ≥{majority}/{n_valid_periods} periods.")
        else:
            candidate_pool = {}
            for period_idx, train_results in all_train_results.items():
                for k, v in train_results.items():
                    candidate_pool.setdefault(k, {})[period_idx] = v[ranking_metric]
            print(f"   ⚠ Quality filters too strict — falling back to unfiltered scores "
                  f"({len(candidate_pool)} param sets).")

    def _global_score(k):
        scores = list(candidate_pool[k].values())
        if len(scores) < n_valid_periods:
            worst = min(scores)
            scores = scores + [worst] * (n_valid_periods - len(scores))
        return float(np.mean(scores))

    global_best_params_tuple = max(candidate_pool, key=_global_score)
    global_best_avg_score = _global_score(global_best_params_tuple)
    n_periods_qualified = len(candidate_pool[global_best_params_tuple])

    global_best_train_perfs = {}
    for period_idx, train_results in all_train_results.items():
        if global_best_params_tuple in train_results:
            global_best_train_perfs[period_idx] = train_results[global_best_params_tuple]

    global_best_params_dict = next(iter(global_best_train_perfs.values()))['params']

    print(f"\n   ✓ Globally Best Parameters Selected (avg IS {ranking_metric} = {global_best_avg_score:.4f})")
    print(f"   Qualified in {n_periods_qualified}/{n_valid_periods} IS periods")
    print(f"\n   Per-period IS scores for selected params:")
    for period_idx in sorted(all_train_results.keys()):
        if period_idx in global_best_train_perfs:
            gv = global_best_train_perfs[period_idx]
            score = gv[ranking_metric]
            period_scores_sorted = sorted(all_train_results[period_idx].values(),
                                          key=lambda x: x[ranking_metric], reverse=True)
            rank_pos = next((i + 1 for i, v in enumerate(period_scores_sorted)
                             if v is all_train_results[period_idx].get(global_best_params_tuple)), '?')
            print(f"     Period {period_idx + 1}: {ranking_metric}={score:.4f}  "
                  f"rank={rank_pos}/{len(period_scores_sorted)}  "
                  f"avgRet={gv['avg_return_pct']:+.2f}%  "
                  f"sharpe={gv['avg_sharpe']:.3f}  trades={int(gv['total_trades'])}")
        else:
            print(f"     Period {period_idx + 1}: ⚠ param set did not appear (filled with worst-period penalty)")

    print(f"\n   " + "="*66)
    print(f"   ML Settings:")
    print(f"     Neighbors (K):         {global_best_params_dict['neighbors_count']}")
    print(f"     Feature Count:         {global_best_params_dict['feature_count']}")
    ...
    for _fi in range(1, 15):
        _ft = global_best_params_dict.get(f'f{_fi}_type')
        if _ft:
            _pa = global_best_params_dict[f'f{_fi}_param_a']
            _pb = global_best_params_dict.get(f'f{_fi}_param_b', 1)
            if _pb != 1:
                print(f"     F{_fi}: {_ft}({_pa},{_pb}) - {_fn.get(_ft, _ft)}")
            else:
                print(f"     F{_fi}: {_ft}({_pa}) - {_fn.get(_ft, _ft)}")
    print(f"   Filters:")
    print(f"     Volatility:            {'ON' if global_best_params_dict['use_volatility_filter'] else 'OFF'}")
    r_thr = global_best_params_dict.get('regime_threshold', 0)
    r_desc = "block bearish" if r_thr == 0 else "require bullish" if r_thr >= 1 else f"thr={r_thr}"
    print(f"     Regime:                {'ON' if global_best_params_dict['use_regime_filter'] else 'OFF'} (monthly H/L, {r_desc})")
    spy_thr = global_best_params_dict.get('spy_regime_threshold', 0)
    spy_desc = "block bearish" if spy_thr == 0 else "require bullish" if spy_thr >= 1 else f"thr={spy_thr}"
    print(f"     SPY Regime:            {'ON' if global_best_params_dict.get('use_spy_filter', False) else 'OFF'} ({spy_desc})")
    print(f"     Kernel:                {'ON' if global_best_params_dict['use_kernel_filter'] else 'OFF'}")
    print(f"   Exit:")
    print(f"     Bars to Hold:          {global_best_params_dict['bars_to_hold']}")
    _pyr_on = global_best_params_dict.get('use_pyramiding', False)
    print(f"   Pyramiding:              {'ON' if _pyr_on else 'OFF'}")
    print(f"   " + "="*66)

    # ── PASS 2: Validate globally selected params on each OOS period ──────────

    print(f"\n{'='*70}")
    print(f"PASS 2/2: OUT-OF-SAMPLE VALIDATION (ALL PERIODS, GLOBAL PARAMS)")
    print(f"{'='*70}")

    for period_idx, period_info in enumerate(periods):
        train_start, train_end, test_start, test_end = period_info

        if period_idx not in all_train_results:
            print(f"\n   Period {period_idx + 1}: Skipping (no IS results)")
            continue

        best_train_perf = global_best_train_perfs.get(period_idx)
        train_grouped = all_train_grouped.get(period_idx, {})

        print(f"\n{'='*70}")
        print(f"PERIOD {period_idx + 1}/{len(periods)} — OUT-OF-SAMPLE PHASE")
        print(f"{'='*70}")
        print(f"Train: {train_start} to {train_end}")
        print(f"Test:  {test_start} to {test_end}")

        if best_train_perf:
            print(f"\n   In-Sample Performance (globally selected params):")
        ...
        if config.get('use_stock_qualification', False) and global_best_params_tuple in train_grouped:
            best_stock_results = train_grouped[global_best_params_tuple]
            qualified_symbols = []
            disqualified = []
        ...
        print(f"   Testing globally selected parameters on unseen {test_start} to {test_end} data...")
        ...
        test_results, equity_data, _ = optimize_single_period(
            data_cache, test_symbols, [global_best_params_tuple], period_idx, 'test', config,
            capture_equity=True
        )

        if global_best_params_tuple in test_results:
            test_performance = test_results[global_best_params_tuple]
            ...
            if best_train_perf and abs(degradation_pct) > 50:
                print(f"     ⚠️  WARNING: High degradation suggests overfitting!")
            elif best_train_perf and abs(degradation_pct) < 20:
            ...
            if equity_data:
                ...
                _kelly_stats = None
                _ranking_stats = None
                if global_best_params_tuple in train_grouped:
                    _kelly_stats = {}
                    _ranking_stats = {}
                    for sr in train_grouped[global_best_params_tuple]:
            ...
            is_return = best_train_perf['sum_return_pct'] if best_train_perf else 0.0
            is_avg_return = best_train_perf['avg_return_pct'] if best_train_perf else 0.0
            is_sharpe = best_train_perf['avg_sharpe'] if best_train_perf else 0.0
            is_drawdown = best_train_perf['avg_max_drawdown'] if best_train_perf else 0.0
            is_trades = best_train_perf['total_trades'] if best_train_perf else 0
            is_win_rate = best_train_perf['avg_win_rate'] if best_train_perf else 0.0
            is_pf = best_train_perf['avg_profit_factor'] if best_train_perf else 0.0
            is_ml_acc = best_train_perf['ml_accuracy'] if best_train_perf else 0.0
            is_ml_bull = best_train_perf['ml_bullish_accuracy'] if best_train_perf else 0.0
            is_ml_bear = best_train_perf['ml_bearish_accuracy'] if best_train_perf else 0.0
            is_ml_preds = best_train_perf['total_ml_predictions'] if best_train_perf else 0
            ...
            period_result = {
                ...
                'in_sample_avg_return': is_avg_return,
                ...
                'in_sample_ml_accuracy': is_ml_acc,
                'in_sample_ml_bullish_accuracy': is_ml_bull,
                'in_sample_ml_bearish_accuracy': is_ml_bear,
                'in_sample_ml_predictions': is_ml_preds,
                ...
                'trade_difference': int(test_performance['total_trades']) - int(is_trades),
                'params': global_best_params_dict,
...
def print_walkforward_results(df_results):
    ...
    print("\n🔹 PYRAMIDING:")
    print(f"   use_pyramiding:           {params.get('use_pyramiding', False)}")
    print(f"   max_pyramid_entries:      {params.get('max_pyramid_entries', 2)}")
    print(f"   pyramid_size_pct:         {params.get('pyramid_size_pct', Decimal('0.5'))}")
    ...
    print(f"    'use_pyramiding': {params.get('use_pyramiding', False)},")
    print(f"    'max_pyramid_entries': {params.get('max_pyramid_entries', 2)},")
    print(f"    'pyramid_size_pct': Decimal('{params.get('pyramid_size_pct', Decimal('0.5'))}'),")
    print("\n    # Other")

...
CSV_FILE = '../classification_set.csv'
PERIOD = "1y"
INITIAL_CASH = 10_000
PLOT_BEST = True
# ============================================================================
...
def run_multi_backtest(csv_file='stocks.csv', period="2y", initial_cash=10_000,
    ...
    # Build universe from CSV symbols for auto-peer selection
    _sector_map = None
    if strategy_params and strategy_params.get('use_cross_symbol_training', False):
        if strategy_params.get('cross_symbol_auto_peers', False):
            strategy_params['cross_symbol_etfs'] = ','.join(symbols)
            # Pre-fetch all sectors upfront to avoid per-stock API latency
            print(f"\nPre-fetching sector data for {len(symbols)} symbols...")
            from cross_symbol_preloader import prefetch_sectors, find_sector_peers, get_ticker_sector
            _sector_map = prefetch_sectors(symbols)
    ...
    if strategy_params:
        ...
        if strategy_params.get('use_cross_symbol_training', False):
            if strategy_params.get('cross_symbol_auto_peers', False):
                ...
                print(f"    - Universe: {len(strategy_params.get('cross_symbol_etfs', '').split(','))} symbols (from CSV)")
            else:
    ...
    for symbol in symbols:
        ...
        if _sector_map is not None:
            ...
            peers = find_sector_peers(symbol, symbols, max_peers)

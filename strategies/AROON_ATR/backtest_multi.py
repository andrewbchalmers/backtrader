...
CSV_FILE = '../../live_trader/watchlist.csv'
PERIOD = "2y"
INITIAL_CASH = 10_000
PLOT_BEST = True
# ============================================================================
...
def run_multi_backtest(csv_file='stocks.csv', period="2y", initial_cash=10_000,
    ...
    for idx, row in top5.iterrows():
        print(f"   {row['symbol']:6s}: {row['return_pct']:7.2f}% | vs SPY: {row['outperformance']:+7.2f}% | Sharpe: {row['sharpe']:5.2f} | Trades: {int(row['total_trades'])}")
    ...
    best_sharpe = df_results.nlargest(5, 'sharpe')[['symbol', 'sharpe', 'return_pct', 'max_drawdown']]

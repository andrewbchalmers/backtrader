...
class StrategyOptimizer:
    ...
    def filter_strategies(self, strategy_results):
        ...
        print(f"\nFiltering {len(strategy_results)} strategies...")
        print("First 3 strategies' metrics:")

        for i, (strategy_config, metrics) in enumerate(strategy_results[:3]):
            if metrics:
                print(f"\nStrategy {strategy_config['id']}: {strategy_config['description']}")
        ...
        print(f"\n✓ {len(filtered)} strategies passed filters (out of {len(strategy_results)})")
        return filtered
    ...
    def _meets_requirements(self, metrics):
        ...
        if metrics['total_trades'] < min_trades:
            print(f"  Filtered: total_trades={metrics['total_trades']} < {min_trades}")
            return False

        # Maximum drawdown
        max_dd = self.filters.get('max_drawdown', 0.30)
        if abs(metrics['max_drawdown']) > max_dd * 100:
            print(f"  Filtered: max_drawdown={abs(metrics['max_drawdown']):.2f}% > {max_dd*100}%")
            return False

        # Minimum Sharpe ratio
        min_sharpe = self.filters.get('min_sharpe', 0.5)
        if metrics['sharpe_ratio'] < min_sharpe:
            print(f"  Filtered: sharpe_ratio={metrics['sharpe_ratio']:.3f} < {min_sharpe}")
            return False

        # Minimum profit factor
        min_pf = self.filters.get('min_profit_factor', 1.2)
        if metrics['profit_factor'] < min_pf:
            print(f"  Filtered: profit_factor={metrics['profit_factor']:.2f} < {min_pf}")
            return False

        # Minimum win rate
        min_wr = self.filters.get('min_win_rate', 0.35)
        if metrics['win_rate'] < min_wr:
            print(f"  Filtered: win_rate={metrics['win_rate']:.2%} < {min_wr:.2%}")
            return False

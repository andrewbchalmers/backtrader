...
class PerformanceAnalyzer:
    ...
    def calculate_metrics(self):
        ...
        metrics = {
            ...
            'stocks_traded': self._count_stocks_traded(),
    ...
    def _calculate_total_return(self):
        """Calculate total portfolio return across all stocks that actually traded"""
        # Only include stocks where trades actually occurred
        traded_results = [r for r in self.results if r.get('total_trades', 0) > 0]

        if not traded_results:
            return 0.0

        # Calculate actual portfolio return based on total P&L from traded stocks
        total_pnl = sum(r['pnl'] for r in traded_results)
        total_starting_value = sum(r['starting_value'] for r in traded_results)

        if total_starting_value == 0:
            return 0.0

        return (total_pnl / total_starting_value) * 100
    ...
    def _count_stocks_traded(self):
        """Count number of stocks where trades actually occurred"""
        return sum(1 for r in self.results if r.get('total_trades', 0) > 0)
